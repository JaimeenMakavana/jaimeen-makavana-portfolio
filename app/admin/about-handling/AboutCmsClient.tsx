...
import { syncAboutMilestones } from "./actions";
import type { AboutMilestone } from "@/app/lib/about/repository";

// --- TYPES ---
type CareerMilestone = AboutMilestone;
...
export default function AboutCmsClient({
  initialMilestones,
}: {
  initialMilestones: CareerMilestone[];
}) {
  const router = useRouter();
  const [isRefreshing, startRefreshTransition] = useTransition();
  const [isSyncing, startSyncTransition] = useTransition();
  const [milestones, setMilestones] = useState<CareerMilestone[]>(
    initialMilestones.length > 0 ? initialMilestones : INITIAL_MILESTONES,
  );
